...
📌 Utility Hub에서 더 자세한 분석을 확인하세요!`;
    ...
    const sajuFaqs = [
        { q: "사주풀이는 무료인가요?", a: "네, 본 명리 서비스는 100% 무료이며 별도의 회원가입 없이 이용하실 수 있습니다." },
        { q: "태어난 시간을 모르면 어떻게 하나요?", a: "태어난 시간을 모를 경우 '모름'을 선택하시면 년, 월, 일의 3주로 분석해 드립니다. 다만 전체적인 해석의 정확도는 낮아질 수 있습니다." }
    ];

    const sajuSteps = [
        "자신의 성별을 선택하세요 (남성 또는 여성).",
        "달력 도구를 클릭하여 정확한 양력 생년월일을 선택하세요.",
        "태어난 시간(시/분)을 입력하세요. 모를 경우 '모름'을 체크하세요.",
        "'분석하기' 버튼을 누르면 즉시 사주 팔자와 오행, 신살 분석 결과가 나옵니다."
    ];
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            <ToolGuide
                title="무료 만세력 (사주팔자)"
                intro="만세력은 사람이 태어난 연, 월, 일, 시의 네 가지 기둥(사주)과 여덟 글자(팔자)를 통해 타고난 운명과 성향을 분석하는 명리학의 가장 기본적이 도구입니다. 이 도구는 전통 명리학 이론을 현대적으로 해석하여 누구나 쉽게 자신의 삶의 흐름을 이해할 수 있도록 돕습니다."
                steps={[
                    "본인의 정확한 생년월일을 입력하세요 (양력/음력 구분 필수).",
                    "태어난 시간을 입력하세요. 시간을 모를 경우 '시주' 분석이 제한될 수 있습니다.",
                    "성별을 선택하세요. 대운(인생의 큰 흐름)의 방향이 성별과 태어난 해의 음양에 따라 달라집니다.",
                    "'사주팔자 확인하기'를 눌러 오행 분석, 대운, 신살 등 상세 정보를 확인하세요."
                ]}
                tips={[
                    "일간(본원)은 당신을 상징하는 핵심 글자입니다. 일간의 오행이 무엇인지 먼저 확인해보세요.",
                    "대운은 10년마다 바뀌는 큰 운의 흐름입니다. 현재 본인이 어떤 대운의 단계에 있는지 체크하는 것이 중요합니다.",
                    "신살 중 천을귀인이나 문창귀인이 있다면 본인의 재능이나 귀인의 도움을 받을 수 있는 분야를 찾아보세요.",
                    "오행 통계에서 부족한 기운이 있다면 그 기운에 해당하는 색상이나 취미 활동으로 보완하는 것을 추천합니다."
                ]}
                faqs={[
                    { q: "태어난 시간을 모르면 정확하지 않나요?", a: "시간을 모르면 사주 8자 중 2자(시주)를 알 수 없어 정확도가 떨어지지만, 나머지 6자만으로도 충분히 유의미한 분석이 가능합니다." },
                    { q: "대운은 좋은 건가요 나쁜 건가요?", a: "대운은 단순히 좋은 운이 아니라 '인생의 기상 조건'과 같습니다. 환경에 맞춰 어떻게 대비하느냐가 더 중요합니다." },
                    { q: "음력 생일은 윤달 체크가 중요한가요?", a: "네, 옛날 달력에서는 윤달에 따라 절기가 달라지므로 정확한 사주 분석을 위해 윤달 여부를 반드시 확인해야 합니다." },
                    { q: "여기서 나오는 해석은 믿을만한가요?", a: "본 도구는 공신력 있는 명리학 알고리즘(lunar-javascript)을 바탕으로 제작되었으나, 최종적인 결정은 본인의 몫임을 잊지 마세요." }
                ]}
            />
        </div>
    );
